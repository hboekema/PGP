...
class Trainer:
    ...
    def __init__(
        ...
        train_set = initialize_dataset(
            ds_type, ["load_data", data_dir, cfg["train_set_args"]] + spec_args
        )
        val_set = initialize_dataset(
            ds_type, ["load_data", data_dir, cfg["val_set_args"]] + spec_args
        )
        datasets = {"train": train_set, "val": val_set}
        self.tr_ds = train_set
        self.val_ds = val_set
        ...
        self.optimizer = torch.optim.AdamW(
            self.model.parameters(), lr=cfg["optim_args"]["lr"]
        )
        self.scheduler = torch.optim.lr_scheduler.StepLR(
            self.optimizer,
            step_size=cfg["optim_args"]["scheduler_step"],
        ...
        if (
            hasattr(self.model.decoder, "anchors")
            and torch.as_tensor(self.model.decoder.anchors == 0).all()
        ):
            print()
    ...
    def train(self, num_epochs: int, output_dir: str):
        ...
        for epoch in range(start_epoch, start_epoch + num_epochs):
            ...
            # Store example visualizations
            # only show for train set for moment
            if epoch % self.vis_period == 0:
                self.log_visualizations(self.tr_ds, "train")
            ...
            self.val_metric = (
                val_epoch_metrics[self.val_metrics[0].name]
                / val_epoch_metrics["minibatch_count"]
            )
            ...
            self.save_checkpoint(
                os.path.join(
                    output_dir, "checkpoints", str(self.current_epoch) + ".tar"
                )
            )

    def log_visualizations(self, ds, mode):
        max_idx = 200
        idcs = range(0, max_idx, int(max_idx / self.num_vis))
        ...
        for idx in idcs:
            ...
            image.save(
                os.path.join(
                    ...
                    "vis",
                    mode,
                    f"E{self.current_epoch:05d}_{idx:03d}.png",
                )
            )
    ...
    def print_metrics(self, epoch_metrics: Dict, dl: torch_data.DataLoader, mode: str):
        ...
        eta = (
            epoch_metrics["time_elapsed"] / epoch_metrics["minibatch_count"]
        ) * minibatches_left
